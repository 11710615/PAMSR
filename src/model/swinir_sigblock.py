...
class SwinIR(nn.Module):
    ...
    def __init__(self, img_size=48, patch_size=1, in_chans=3,
        ...
        ##########
        # sigma block: multi-task
        conv_part = []
        up_part = []
        for _ in range(4):
            conv_part.append(nn.Conv2d(embed_dim, embed_dim, kernel_size=3, padding=1))
            conv_part.append(nn.PReLU(embed_dim))  # each channel with different parameter
        self.conv_part = nn.Sequential(*conv_part)

        up_part.append(nn.Conv2d(embed_dim, num_feat, 3, 1, 1))
        up_part.append(nn.LeakyReLU(inplace=True))
        up_part.append(Upsample(upscale, num_feat))
        up_part.append(nn.Conv2d(num_feat, num_out_ch, 3, 1, 1))
    ...
    def forward(self, x, mode='train'):
        ...
        # add a sigma block branch
            y = self.conv_part(x) + x
            y = self.up_part(y)
