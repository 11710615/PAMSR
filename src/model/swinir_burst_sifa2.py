...
class EBFA(nn.Module):
    ...
    def __init__(self, input_nc, burst_size):
        ...
        # # feature fusion
        # conv_burst = [nn.Conv2d(self.burst_size, self.num_features // 2, 3, 1, 1)] 
        # conv_burst = nn.Sequential(*conv_burst)
        # self.conv_burst = conv_burst
        # conv_burst_1 = [nn.Conv2d(self.num_features // 2**(i), self.num_features // 2**(i+1), 3, 1, 1) for i in range(1,4)]
        # conv_burst_1 += [nn.Conv2d(self.num_features // 16, self.burst_size, 3, 1, 1)]
    ...
    #     x = self.con_burst_1(x) # [180,90,256,256]->[180,5,256,256]
