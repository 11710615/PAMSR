...
######################################### no_pre_train##################################
    if args.template.find('polar_swinir_L1_nopretrain_downgt_x2') >= 0:
        args.model = 'Swinir'
...
########################################## swinir_L1 #################################################
    if args.template.find('polar_swinir_L1_downgt_x2') >= 0:
        ...
        # args.loss = '200*Gradient_L1+0.5*VGG54'  # 10L1? 40:bury
        args.loss = '1*RL1'
        args.save = str(args.downsample_gt)+'_'+'polar_swinir_L1_downgt_x2_burst-' +str(args.burst_size)+ '_' + args.loss
        args.pre_train = '../experiment1/pretrain_model/003_realSR_BSRGAN_DFO_s64w8_SwinIR-M_x2_GAN.pth'
...
################################### swinir_GradientL1 ##################################
    if args.template.find('polar_swinir_GradientL1_downgt_x2') >= 0:
        ...
        args.data_test = 'burst_v1'
        args.rgb_range = 1
        args.downsample_gt = True  # control traning and val set
        if args.downsample_gt:
            down_scale = 2
        else:
            down_scale = 1
        ...
        # args.loss = '200*Gradient_L1+0.5*VGG54'  # 10L1? 40:bury
        args.loss = '1*Gradient_L1'
        args.save = str(args.downsample_gt)+'_'+'polar_swinir_GradientL1_downgt_x2_burst-' +str(args.burst_size)+ '_' + args.loss
        args.pre_train = '../experiment1/pretrain_model/003_realSR_BSRGAN_DFO_s64w8_SwinIR-M_x2_GAN.pth'
        # args.pre_train = '../experiment/'+str(args.downsample_gt)+'_'+'polar_swinir_GradientL1_downgt_x2_burst-' + str(args.burst_size) + '_' + args.loss + '/model/model_best.pt'
        args.no_augment = True
