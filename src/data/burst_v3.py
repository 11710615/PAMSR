...
class BurstSRDataset(torch.utils.data.Dataset):
    ...
    def _get_burst_list(self, data_id):
        burst_list = os.listdir(self.root)  # 'busrst_data/train'
        #print(burst_list)
        if self.split == 'train':
            data_id = data_id[0]
        else:
    ...
    def _get_crop(self, frames, gt, patch_size=(64,64), scale=2, center_crop=False, entrop_select=False):
        ...
        if center_crop:
            ...
            if entrop_select:
                ...
                img_entrop = 0
                k=0
                for i_x in range(ix_start, ix_start + ip[1], 8):  # select 8 adjent pathces
                    img = frames[0][iy:iy + ip[0], i_x:i_x + ip[1]]
                    k=k+1
                    # print('**',k, ix_start, i_x, image_entrop_gray(img))
    ...
    def __getitem__(self, index):
        ...
        if self.split == 'train':
            ret, patch_cord = self._get_crop(frames, gt, patch_size=self.args.patch_size, scale=self.args.scale[0], entrop_select=self.args.entrop_select)
            burst_list, gt = ret
