...
class CannyFilter(nn.Module):
    ...
    def forward(self, img, low_threshold=0.15, high_threshold=0.6, hysteresis=True):
        ...
        return grad_magnitude, thin_edges.float()

class GradientLoss(nn.Module):
    def __init__(self, rec=False, operator='sobel'):
    ...
    def forward(self, sr, hr):
        ...
        *_, h, w = sr.shape
        if self.rec:
            ...
            sr_rec_grad, sr_rec_canny = self.canny_filter(sr_rec)

            hr_rec = self.rec_img(hr, patch_cord)
            hr_rec_grad, hr_rec_canny = self.canny_filter(hr_rec)
            ...
            if self.operator == 'sobel':
                ...
                out = self.L1(sr_rec_canny, hr_rec_canny)
            out = out * 1024 * 1024 / h / w
        else:
            sr_grad, sr_canny = self.canny_filter(sr)
            hr_grad, hr_canny = self.canny_filter(hr)
            # print('sr***', sr.shape, sr_rec.shape, hr_rec.shape)
            if self.operator == 'sobel':
                out = self.L1(sr_grad, hr_grad)
            elif self.operator == 'canny':
                out = self.L1(sr_canny, hr_canny)
        # print('loss**', out)
        return out
