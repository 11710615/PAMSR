...
class spwl1(nn.Module):
    def __init__(self, whether_rec=True):
        super(spwl1, self).__init__()
        self.L1 = nn.L1Loss()
    ...
    def forward(self, sr, hr):
        ...
        *_, h, w = sr.shape
        ...
        # print(hr_grad.shape, sp_map.shape, '***')
        hr_grad_SP = torch.mul(hr_grad, sp_map)
        # normalize
        hr_grad_SP = hr_grad_SP - torch.min(hr_grad_SP) / (torch.max(hr_grad_SP) - torch.min(hr_grad_SP))
        
        out = self.L1(torch.mul(sr, hr_grad_SP), torch.mul(hr, hr_grad_SP))
